...
public class JFrame08 extends JFrame {
    ...
    public JFrame08( String title ){
        ...
        Runtime.getRuntime().addShutdownHook( new Shutdown1() ); // シャットダウンフックの設定1
        Runtime.getRuntime().addShutdownHook( new Shutdown2() ); // シャットダウンフックの設定2
        addWindowListener( new WindowEventHandler() );
        setSize(500, 300);
        setVisible(true);
    }
    class Shutdown1 extends Thread { // Thread のサブクラスとして定義
        public void run() {
            System.out.println( "シャットダウンフック１が呼び出されました" );
        }
    }
    class Shutdown2 extends Thread { // Thread のサブクラスとして定義
        public void run() {
