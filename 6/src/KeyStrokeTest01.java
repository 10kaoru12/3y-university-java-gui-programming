...
public class KeyStrokeTest01 extends JFrame {
  ...
  public KeyStrokeTest01( String title ){
    ...
    Action actionUp    = new ActionUp(); // 各アクションの生成
    Action actionDown  = new ActionDown();
    Action actionRight = new ActionRight();
    Action actionLeft  = new ActionLeft();

    JButton buttonUp = new JButton( actionUp ); // 各ボタンへのアクションの設定とペインへの追加
    ...
    InputMap imap = pane.getInputMap( JComponent.WHEN_IN_FOCUSED_WINDOW ); // グローバルなキーストロークを得る
    ActionMap amap = pane.getActionMap();

    imap.put( KeyStroke.getKeyStroke("UP"), actionUp ); // 各キーストロークとアクションの設定
